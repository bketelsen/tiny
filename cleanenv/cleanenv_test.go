...
two = 2`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tmpFile, err := os.CreateTemp(t.TempDir(), fmt.Sprintf("*.%s", tt.ext))
			if err != nil {
...
func TestReadConfig(t *testing.T) {
	type config struct {
		...
		NoDefault string `yaml:"no_default" env:"TEST_NO_DEFAULT"`
		NoEnv     string `yaml:"no_env" env-default:"default"`
	}
